...
<style>
    ...
    /* Top header bar (was showing as a white strip) */
    ...
    /* Widget labels (slider/input captions) were low-contrast gray-on-black */
    ...
    /* Text inputs and number inputs — dark fields instead of default white */
    ...
    /* Sliders — restyle track/thumb to accent color instead of default red */
    ...
    /* Alert boxes (info/warning/error) — were default light-blue, clashed with dark bg */
...
st.sidebar.markdown('<div class="section-label">Growth Assumption</div>', unsafe_allow_html=True)
conservative_mode = st.sidebar.toggle(
    "Cap growth at long-run average",
    value=True,
    help=(
        "Recent history can overstate what a stock will do going forward, "
        "especially after a big run-up. When on, each asset's assumed annual "
        "return is capped at the value below, instead of using its raw "
        "historical average — volatility (risk) is still drawn from actual history."
    ),
)
capped_annual_return = st.sidebar.slider(
    "Capped annual return (%)", min_value=0, max_value=20, value=8, step=1,
    disabled=not conservative_mode,
    help="8-10% roughly tracks the long-run historical average of the S&P 500.",
)
...
if run_button:
    ...
    # Optionally cap each asset's assumed daily drift at a long-run average,
    # so a recent hot streak (e.g. a single high-flying stock) doesn't get
    # extrapolated forward at an unrealistic pace. Volatility/covariance is
    # left untouched — only the assumed average growth rate is capped.
    if conservative_mode:
        capped_daily_return = (1 + capped_annual_return / 100) ** (1 / 252) - 1
        mean_returns = np.minimum(mean_returns, capped_daily_return)
    ...
    if conservative_mode:
        raw_annual = (daily_returns.mean().values * 252) * 100
        capped_tickers = [t for t, r in zip(tickers, raw_annual) if r > capped_annual_return]
        if capped_tickers:
            st.markdown(
                f'<div style="color:{MUTED};font-size:0.85rem;margin-bottom:1rem;">'
                f'Capping applied to <b>{", ".join(capped_tickers)}</b> — their recent historical '
                f'return exceeded {capped_annual_return}%/yr, so growth was capped for this projection. '
                f'Volatility is still based on real history.</div>',
                unsafe_allow_html=True
            )
